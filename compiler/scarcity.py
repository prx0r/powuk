"""UK physical scarcity compiler.

Takes raw constraint data and trade capacity data,
produces a derived scarcity view per region × constraint type.
...
def compile_grid_constraint(
    ...
    if demand_mw is not None and capacity_mw is not None:
        headroom = capacity_mw - demand_mw
    ...
    if queue_count is not None:
        severity = compute_severity(float(queue_count), 10.0)  # baseline 10 pending
        constraints.append(Constraint(
...
def compile_scarsity_view(
    ...
    Returns a dict keyed by region code, each containing
    constraints and a composite scarcity score.
    """
    view = {}
    ts = datetime.utcnow()
